...
//    	wrStream = pc.getRemoteStreams()[0];
    	console.log('my stream' + URL.createObjectURL(wrStream));
    }
    ...
    function localvideourl(sdpOfferCallback){
    	var local = {};
    	local.url = URL.createObjectURL(wrStream);
    	local.userid = that.getGlobalID();
    	local.type = 'local';
    ...
    this.localVideoURL = function (){
    	return localvideourl(that.publishVideoCallback);
    }
