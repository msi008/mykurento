...
;(function($){
    ...
    PptList.prototype = {
        ...
        show : function(){
            var that = this;
            var left = this.el.offset().left-240;
            var top = this.el.offset().top;
            !this.isshow && this.$ppt_list.css({'left':left,'top':top}).addClass("tipScaleIn").show();
            this.isshow = true;
            $('html').on("click", function(){
                var e = event || window.event;
                if($(e.target).closest(".btn-upload").length > 0 || $(e.target).closest(".plupload").length > 0  || $(e.target).closest('[node-type="upload_file"]').length > 0){
                    e.stopPropagation();
                }else{
                    that.hide();
                }
